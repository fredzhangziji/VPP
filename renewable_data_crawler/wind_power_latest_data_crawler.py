...
if __name__ == '__main__': 
    ...
    logger.info(f"每日风力数据爬取开始时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}")
    logger.info(f"每日风力数据爬取结束时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}")
