...
class DayAheadPriceCrawler(JSONCrawler):
    ...
    def save_to_db(self, df, update_columns=None):
        ...
        try:
            ...
            # 保存到数据库
            self.logger.info(f"正在保存 {len(df)} 条记录到表 {self.target_table}")
...
# 异步爬取单个日期的数据
async def crawl_day_ahead_price_for_date(date_str, target_table=None, cookie=None):
    """
    异步爬取指定日期的日前市场出清负荷侧电价数据
    ...
    Returns:
        df: 包含数据的DataFrame，如果失败则返回None
    """
    try:
        # 创建爬虫实例
        crawler = DayAheadPriceCrawler(target_table=target_table, cookie=cookie)
        
        # 获取数据
        return crawler.fetch_data(date_str)
    except Exception as e:
        logger = setup_logger('crawler.day_ahead_price')
        logger.error(f"爬取日期 {date_str} 的数据失败: {e}", exc_info=True)
        return None
...
async def run_historical_crawl(start_date, end_date, target_table=None, cookie=None):
    ...
    try:
        ...
        if df.empty:
            logger = setup_logger('crawler.day_ahead_price')
            logger.warning(f"未获取到 {start_date} 至 {end_date} 的数据")
            return False
...
async def run_daily_crawl(target_table=None, retry_days=3, cookie=None):
    ...
    """
    logger = setup_logger('crawler.day_ahead_price')
    logger.info("开始爬取每日日前市场出清负荷侧电价数据")
    
    # 获取当前日期
    today = datetime.now()
    
    # 创建日期范围（仅当天）
    # 注意：对于日前电价数据，通常只需要获取当天数据即可
    date_range = [today.strftime('%Y-%m-%d')]
    
    # 如果设置了retry_days，则添加前几天的日期
    if retry_days > 1:
        for i in range(1, retry_days):
    ...
    try:
        ...
        for date_str in date_range:
            logger.info(f"正在爬取 {date_str} 的数据")
            ...
            if not df.empty:
                ...
                if success:
                    logger.info(f"成功保存 {date_str} 的数据")
                    return True
                else:
                    logger.warning(f"保存 {date_str} 的数据失败")
            else:
                logger.warning(f"未获取到 {date_str} 的数据")
        
        logger.warning(f"未能成功爬取最近 {retry_days} 天的数据")
        return False
    except Exception as e:
        logger.error(f"爬取每日数据失败: {e}", exc_info=True)
        return False 