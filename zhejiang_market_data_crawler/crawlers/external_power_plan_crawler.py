...
class ExternalPowerPlanCrawler(JSONCrawler):
    """外来电受电计划爬虫"""
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        """
        super().__init__('external_power_plan')
        ...
        # 数据库字段名
        self.field_name = "external_power_plan"  # 外来电受电计划(日)(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条外来电受电计划记录")
        ...
        for item_data in data_list:
            # 检查是否为浙江电网的数据
            dev_name = item_data.get('devName', '')
            if dev_name == '浙江电网' or '浙江' in dev_name:
                # 创建时间点和外来电受电计划值列表
                timestamps = []
                plan_values = []
                ...
                for i in range(1, 97):
                    ...
                    if key in item_data and item_data[key] is not None:
                        ...
                        plan_values.append(float(item_data[key]))
                
                # 如果有数据，返回DataFrame
                if timestamps:
                    df = pd.DataFrame({
                        'date_time': timestamps,
                        self.field_name: plan_values
                    })
                    self.logger.info(f"成功解析浙江电网的数据，共 {len(timestamps)} 条记录")
                    return df
        
        # 如果没有找到浙江电网的数据
        self.logger.warning("未能找到浙江电网的外来电受电计划数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        # 请求URL - 外来电受电计划数据的URL
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/planOut/outPowerPlan"
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取外来电受电计划数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的外来电受电计划数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的外来电受电计划数据")
                
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的外来电受电计划数据失败: {e}", exc_info=True)
        ...
        if all_data:
            ...
            self.logger.info(f"共获取 {len(result)} 条外来电受电计划数据记录")
            return result
        else:
            self.logger.warning("未获取到任何外来电受电计划数据")
            return pd.DataFrame()
...
async def crawl_external_power_plan_for_date(date_str, target_table=None, cookie=None):
    """
    获取指定日期的外来电受电计划数据
    ...
    Returns:
        success: 是否获取成功
    """
    crawler = ExternalPowerPlanCrawler(target_table=target_table, cookie=cookie)
