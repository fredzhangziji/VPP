...
class NonMarketTotalOutputCrawler(JSONCrawler):
    """非市场机组实时总出力爬虫"""
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        """
        super().__init__('non_market_total_output')
        ...
        # 浙江电网的非市场机组实时总出力字段名
        self.field_name = "non_market_total_output"  # 非市场机组实时总出力
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条非市场机组实时总出力记录")
        ...
        if all_data:
            ...
            self.logger.info(f"成功解析浙江电网的数据，共 {len(combined_df)} 条记录")
            return combined_df
        
        # 如果没有找到浙江电网的数据
        self.logger.warning("未能找到浙江电网的非市场机组实时总出力数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        # 请求URL - 非市场机组实时总出力使用的URL
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/schedule/noMarketUnitLoad"
        ...
        payload = {
            ...
            "data": {
                ...
                "zjNumber": "0200092",  # 非市场机组实时总出力使用的zjNumber
                "measType": None
            }
        }
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        try:
            ...
            for date_str in date_range:
                self.logger.info(f"获取 {date_str} 的非市场机组实时总出力数据")
    ...
    def run(self, start_date=None, end_date=None, retry_days=3):
        """
        运行爬虫
        ...
        """
        try:
            self.logger.info(f"运行非市场机组实时总出力爬虫，时间范围: {start_date} 至 {end_date}")
            
            # 如果指定了日期范围，则抓取该范围的数据
            if start_date and end_date:
                df = self.fetch_data(start_date, end_date)
                if not df.empty:
                    self.logger.info(f"成功获取 {start_date} 至 {end_date} 的数据")
                    return self.save_to_db(df)
                else:
                    self.logger.warning(f"未获取到 {start_date} 至 {end_date} 的数据")
                    return False
            # 如果仅指定了开始日期，则只抓取该日的数据
            elif start_date:
                df = self.fetch_data(start_date)
                if not df.empty:
                    self.logger.info(f"成功获取 {start_date} 的数据")
                    return self.save_to_db(df)
                else:
                    self.logger.warning(f"未获取到 {start_date} 的数据")
                    return False
            # 如果未指定日期，则抓取当天的数据
            else:
                # 获取当前日期
                ...
                for i in range(retry_days):
                    ...
                    self.logger.info(f"尝试获取 {date_str} 的数据")
                    
                    # 获取数据
                    df = self.fetch_data(date_str)
                    
                    if not df.empty:
                        self.logger.info(f"成功获取 {date_str} 的数据")
                        return self.save_to_db(df)
                
                self.logger.warning(f"未能成功抓取最近 {retry_days} 天的数据")
                return False
        except Exception as e:
            self.logger.error(f"运行爬虫异常: {e}", exc_info=True)
            return False

# 异步函数，用于抓取指定日期的非市场机组实时总出力数据
async def crawl_non_market_total_output_for_date(date_str, target_table=None, cookie=None):
    """
    抓取指定日期的非市场机组实时总出力数据
    ...
    Returns:
        success: 是否抓取成功
    """
    logger = setup_logger('crawl_non_market_total_output')
    logger.info(f"抓取 {date_str} 的非市场机组实时总出力数据")
    
    try:
        # 创建爬虫实例
        crawler = NonMarketTotalOutputCrawler(target_table=target_table, cookie=cookie)
        ...
        if not df.empty:
            ...
            logger.warning(f"未获取到 {date_str} 的非市场机组实时总出力数据")
            return False
    except Exception as e:
        logger.error(f"抓取数据异常: {e}", exc_info=True)
        return False

# 异步函数，用于抓取历史范围内的非市场机组实时总出力数据
async def run_historical_crawl(start_date, end_date, target_table=None, cookie=None):
    """
    抓取历史范围内的非市场机组实时总出力数据
    ...
    Returns:
        success: 是否抓取成功
    """
    logger = setup_logger('historical_non_market_total_output')
...
async def run_daily_crawl(target_table=None, retry_days=3, cookie=None):
    ...
    Returns:
        success: 是否抓取成功
    """
    logger = setup_logger('daily_non_market_total_output')
    logger.info("开始每日抓取非市场机组实时总出力数据")
    ...
    for i in range(retry_days):
        ...
        # 抓取数据
        success = await crawl_non_market_total_output_for_date(date_str, target_table=target_table, cookie=cookie)
