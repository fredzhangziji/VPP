...
# 异步函数，用于抓取指定日期的水电实时总出力数据
async def crawl_actual_hydro_output_for_date(date_str, target_table=None, cookie=None):
    """
    抓取指定日期的水电实时总出力数据
    ...
    Returns:
        success: 是否抓取成功
    """
    logger = setup_logger('crawl_actual_hydro_output')
    logger.info(f"抓取 {date_str} 的水电实时总出力数据")
    
    try:
        # 创建爬虫实例
        crawler = ActualHydroOutputCrawler(target_table=target_table, cookie=cookie)
        ...
        if not df.empty:
            ...
            logger.warning(f"未获取到 {date_str} 的水电实时总出力数据")
            return False
    except Exception as e:
        logger.error(f"抓取 {date_str} 的水电实时总出力数据异常: {e}", exc_info=True)
        return False
...
async def run_daily_crawl(target_table=None, retry_days=3, cookie=None):
    ...
    for i in range(retry_days):
        ...
        # 抓取数据
        success = await crawl_actual_hydro_output_for_date(date_str, target_table=target_table, cookie=cookie)
