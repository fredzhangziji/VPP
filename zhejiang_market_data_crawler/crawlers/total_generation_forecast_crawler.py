...
class TotalGenerationForecastCrawler(JSONCrawler):
    """发电总出力预测爬虫"""
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        """
        super().__init__('total_generation_forecast')
        ...
        # 数据库字段名
        self.field_name = "total_generation_forecast"  # 发电总出力预测(日)(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条发电总出力预测记录")
        ...
        # 如果没有找到浙江电网的数据
        self.logger.warning("未能找到浙江电网的发电总出力预测数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        # 请求URL - 发电总出力预测数据的URL
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/supplyAndDemand/powerAll"
        ...
        payload = {
            ...
            "data": {
                ...
                "zjNumber": "0200007",  # 发电总出力预测使用的zjNumber
                "measType": None
            }
        }
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取发电总出力预测数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的发电总出力预测数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的发电总出力预测数据")
                
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的发电总出力预测数据失败: {e}", exc_info=True)
        ...
        if all_data:
            ...
            self.logger.info(f"共获取 {len(result)} 条发电总出力预测数据记录")
            return result
        else:
            self.logger.warning("未获取到任何发电总出力预测数据")
            return pd.DataFrame()
...
async def crawl_total_generation_forecast_for_date(date_str, target_table=None, cookie=None):
    """
    获取指定日期的发电总出力预测数据
    ...
    Returns:
        success: 是否获取成功
    """
    crawler = TotalGenerationForecastCrawler(target_table=target_table, cookie=cookie)
