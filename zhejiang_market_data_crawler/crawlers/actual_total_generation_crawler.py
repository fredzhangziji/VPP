...
class ActualTotalGenerationCrawler(JSONCrawler):
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        Args:
            ...
            cookie: API请求的Cookie
        """
        super().__init__('actual_total_generation')
        ...
        self.field_name = "actual_total_generation"  # 发电实时总出力(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条发电实时总出力记录")
        ...
        for item_data in data_list:
            ...
            for i in range(1, 97):
                ...
                if key in item_data and item_data[key] is not None:
                    ...
                    values.append(float(item_data[key]))
            ...
            if timestamps:
                ...
                self.logger.info(f"成功解析发电实时总出力数据，共 {len(timestamps)} 条记录")
                return df
                
        self.logger.warning("未能找到发电实时总出力数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/schedule/realPowerLoad"
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取发电实时总出力数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的发电实时总出力数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的发电实时总出力数据")
                    
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的发电实时总出力数据失败: {e}", exc_info=True)
                
            current_date += timedelta(days=1)
            
        if all_data:
            result = pd.concat(all_data, ignore_index=True)
            self.logger.info(f"共获取 {len(result)} 条发电实时总出力数据记录")
            return result
        else:
