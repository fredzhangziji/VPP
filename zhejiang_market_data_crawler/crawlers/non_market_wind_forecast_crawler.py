...
class NonMarketWindForecastCrawler(JSONCrawler):
    """
    非市场风电出力预测爬虫
    """
    def __init__(self, target_table=None, cookie=None):
        super().__init__('non_market_wind_forecast')
        ...
        self.field_name = "non_market_wind_forecast"  # 非市场风电出力预测(日)(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        if not data_list:
            ...
            return pd.DataFrame()
        self.logger.info(f"找到 {len(data_list)} 条非市场风电出力预测记录")
        if query_date:
        ...
        for item_data in data_list:
            ...
            if timestamps:
                ...
                self.logger.info(f"成功解析非市场风电出力预测数据，共 {len(timestamps)} 条记录")
                return df
        self.logger.warning("未能找到非市场风电出力预测数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        if not end_date:
            end_date = start_date
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/supplyAndDemand/notMarketWindPower"
        headers = {
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取非市场风电出力预测数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的非市场风电出力预测数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的非市场风电出力预测数据")
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的非市场风电出力预测数据失败: {e}", exc_info=True)
            current_date += timedelta(days=1)
        if all_data:
            result = pd.concat(all_data, ignore_index=True)
            self.logger.info(f"共获取 {len(result)} 条非市场风电出力预测数据记录")
            return result
        else:
            self.logger.warning("未获取到任何非市场风电出力预测数据")
            return pd.DataFrame()
...
async def crawl_non_market_wind_forecast_for_date(date_str, target_table=None, cookie=None):
    crawler = NonMarketWindForecastCrawler(target_table=target_table, cookie=cookie)
