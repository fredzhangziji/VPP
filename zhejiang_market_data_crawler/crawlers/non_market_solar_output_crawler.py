...
class NonMarketSolarOutputCrawler(JSONCrawler):
    ...
    def __init__(self, target_table=None, cookie=None):
        super().__init__('non_market_solar_output')
        ...
        self.field_name = "non_market_solar_output"  # 非市场光伏实时总出力(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条非市场光伏实时总出力记录")
        ...
        for item_data in data_list:
            ...
            if 'devName' in item_data and item_data.get('devName') == '浙江电网':
                ...
                if timestamps:
                    ...
                    self.logger.info(f"成功解析浙江电网的非市场光伏实时总出力数据，共 {len(timestamps)} 条记录")
                    return df
        
        # 如果没有找到浙江电网的数据
        self.logger.warning("未能找到浙江电网的非市场光伏实时总出力数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        # 请求URL - 非市场光伏实时总出力使用的URL
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/schedule/noMarketSolarPower"
        ...
        payload = {
            ...
            "data": {
                ...
                "zjNumber": "0200099",  # 非市场光伏实时总出力使用的zjNumber
                "measType": None
            }
        }
    ...
    def send_request(self, url, method, headers=None, params=None, data=None, cookies=None):
        ...
        try:
            ...
            if method.upper() == 'GET':
                ...
                response = post(url, data=data, json=params, headers=headers, cookies=cookie_dict)
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取 {date_str} 的非市场光伏实时总出力数据")
            ...
            try:
                ...
                if json_data:
                    ...
                    if not df.empty:
                        all_data.append(df)
                        self.logger.info(f"成功获取 {date_str} 的非市场光伏实时总出力数据，共 {len(df)} 条记录")
                    else:
                        self.logger.warning(f"未找到 {date_str} 的非市场光伏实时总出力数据")
                else:
                    self.logger.error(f"获取 {date_str} 的非市场光伏实时总出力数据失败，响应解析错误")
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的非市场光伏实时总出力数据失败: {e}")
        ...
        if all_data:
            ...
            self.logger.info(f"共获取 {len(result)} 条非市场光伏实时总出力数据记录")
            return result
        else:
