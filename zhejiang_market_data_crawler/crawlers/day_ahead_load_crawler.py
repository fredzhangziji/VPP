...
async def crawl_day_ahead_load_for_date(date_str, target_table=None, cookie=None):
    """
    爬取指定日期的日前负荷预测数据
    ...
    """
    # 创建爬虫实例
    crawler = DayAheadLoadCrawler(target_table=target_table, cookie=cookie)
...
async def run_daily_crawl(target_table=None, retry_days=3, cookie=None):
    ...
    Returns:
        success: 爬取是否成功
    """
    logger = setup_logger('day_ahead_load_daily')
    ...
    # 检查是否成功获取数据
    if df.empty:
        logger.error(f"无法获取最近 {retry_days + 1} 天的日前负荷预测数据，爬取失败")
        return False
