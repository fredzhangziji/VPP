...
class NonMarketHydroOutputCrawler(JSONCrawler):
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        """
        super().__init__('non_market_hydro_output')
        ...
        # 浙江电网的非市场水电实时总出力字段名
        self.field_name = "non_market_hydro_output"  # 非市场水电实时总出力(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条非市场水电实时总出力记录")
        ...
        # 如果没有找到浙江电网的数据
        self.logger.warning("未能找到浙江电网的非市场水电实时总出力数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        # 请求URL - 非市场水电实时总出力使用的URL
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/schedule/noMarketWaterPower"
        ...
        payload = {
            ...
            "data": {
                ...
                "zjNumber": "0200102",  # 非市场水电实时总出力使用的zjNumber
                "measType": None
            }
        }
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取非市场水电实时总出力数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的非市场水电实时总出力数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的非市场水电实时总出力数据")
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的非市场水电实时总出力数据失败: {e}", exc_info=True)
        ...
        if all_data:
            ...
            self.logger.info(f"共获取 {len(result)} 条非市场水电实时总出力数据记录")
            return result
        else:
