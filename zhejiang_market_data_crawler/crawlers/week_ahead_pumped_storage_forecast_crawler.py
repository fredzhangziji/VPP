...
class WeekAheadPumpedStorageForecastCrawler(JSONCrawler):
    """周前抽蓄总出力预测爬虫"""
    ...
    def __init__(self, target_table=None, cookie=None):
        ...
        Args:
            ...
            cookie: API请求的Cookie
        """
        super().__init__('week_ahead_pumped_storage_forecast')
        ...
        self.field_name = "week_ahead_pumped_storage_forecast"  # 抽蓄总出力预测(周)(MW)
    ...
    def transform_data(self, json_data, query_date=None):
        ...
        self.logger.info(f"找到 {len(data_list)} 条抽蓄总出力预测记录")
        ...
        for item_data in data_list:
            ...
            if timestamps:
                ...
                self.logger.info(f"成功解析抽蓄总出力预测数据，共 {len(timestamps)} 条记录")
                return df
                
        self.logger.warning("未能找到抽蓄总出力预测数据")
        return pd.DataFrame()
    ...
    def get_request_params(self, start_date=None, end_date=None):
        ...
        url = "https://zjpx.com.cn/px-settlement-infpubquery-phbzj/supplyAndDemand/weekPumpPowerLoad"
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        while current_date <= end:
            ...
            self.logger.info(f"获取抽蓄总出力预测数据: {date_str}")
            ...
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    self.logger.info(f"成功获取 {date_str} 的抽蓄总出力预测数据，共 {len(df)} 条记录")
                else:
                    self.logger.warning(f"未找到 {date_str} 的抽蓄总出力预测数据")
                    
            except Exception as e:
                self.logger.error(f"获取 {date_str} 的抽蓄总出力预测数据失败: {e}", exc_info=True)
                
            current_date += timedelta(days=1)
            
        if all_data:
            result = pd.concat(all_data, ignore_index=True)
            self.logger.info(f"共获取 {len(result)} 条抽蓄总出力预测数据记录")
            return result
        else:
            self.logger.warning("未获取到任何抽蓄总出力预测数据")
            return pd.DataFrame()
...
async def crawl_week_ahead_pumped_storage_forecast_for_date(date_str, target_table=None, cookie=None):
    """
    获取指定日期的周前抽蓄总出力预测数据
    ...
    """
    crawler = WeekAheadPumpedStorageForecastCrawler(target_table=target_table, cookie=cookie)
