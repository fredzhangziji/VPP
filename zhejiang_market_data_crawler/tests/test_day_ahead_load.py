...
from crawlers.day_ahead_load_crawler import (
    DayAheadLoadCrawler, 
    crawl_day_ahead_load_for_date,
...
# 设置日志记录器
logger = setup_logger('test_day_ahead_load')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'day_ahead_load_forecast']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = DayAheadLoadCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算负荷预测字段的最小值和最大值
            if 'day_ahead_load_forecast' in df.columns:
                logger.info(f"day_ahead_load_forecast 范围: {df['day_ahead_load_forecast'].min()} 至 {df['day_ahead_load_forecast'].max()}")
