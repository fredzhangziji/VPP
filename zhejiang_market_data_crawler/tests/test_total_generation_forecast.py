...
from crawlers.total_generation_forecast_crawler import (
    TotalGenerationForecastCrawler, 
    crawl_total_generation_forecast_for_date,
...
# 设置日志记录器
logger = setup_logger('test_total_generation_forecast')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'total_generation_forecast']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = TotalGenerationForecastCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算发电总出力预测字段的最小值和最大值
            if 'total_generation_forecast' in df.columns:
                logger.info(f"total_generation_forecast 范围: {df['total_generation_forecast'].min()} 至 {df['total_generation_forecast'].max()}")
