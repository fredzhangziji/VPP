...
from crawlers.actual_hydro_output_crawler import (
    ActualHydroOutputCrawler, 
    crawl_actual_hydro_output_for_date,
...
# 设置日志记录器
logger = setup_logger('test_actual_hydro_output')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'actual_hydro_output']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = ActualHydroOutputCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算水电实时总出力字段的最小值和最大值
            if 'actual_hydro_output' in df.columns:
                logger.info(f"actual_hydro_output 范围: {df['actual_hydro_output'].min()} 至 {df['actual_hydro_output'].max()}")
