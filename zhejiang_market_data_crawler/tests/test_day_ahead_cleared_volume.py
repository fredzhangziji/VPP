...
from crawlers.day_ahead_cleared_volume_crawler import (
    DayAheadClearedVolumeCrawler, 
    crawl_day_ahead_cleared_volume_for_date,
...
def setup_test_logger():
    """设置测试日志记录器"""
    logger = logging.getLogger('test_day_ahead_cleared_volume')
    if not logger.handlers:
        logger.setLevel(logging.DEBUG)  # 将日志级别设置为DEBUG，以便显示更多调试信息
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'day_ahead_cleared_volume']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = DayAheadClearedVolumeCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算日前市场出清总电量字段的最小值和最大值
            if 'day_ahead_cleared_volume' in df.columns:
                logger.info(f"day_ahead_cleared_volume 范围: {df['day_ahead_cleared_volume'].min()} 至 {df['day_ahead_cleared_volume'].max()}")
