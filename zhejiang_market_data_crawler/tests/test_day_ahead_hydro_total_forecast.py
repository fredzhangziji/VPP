...
from crawlers.day_ahead_hydro_total_forecast_crawler import (
    DayAheadHydroTotalForecastCrawler,
)

# 目标表名和字段名
TABLE_NAME = 'power_market_data'
FIELD_NAME = 'day_ahead_hydro_total_forecast'

# 设置日志记录器
logger = setup_logger('test_day_ahead_hydro_total_forecast')
...
async def main():
    try:
        logger.info("===== 测试水电总出力预测爬虫 =====")
        # 检查数据库结构
        assert check_db_config(), "数据库结构检查未通过"
        # 测试单日数据
        test_date = '2025-06-02'
        crawler = DayAheadHydroTotalForecastCrawler(target_table=TABLE_NAME)
