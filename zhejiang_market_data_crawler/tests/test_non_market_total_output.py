...
from crawlers.non_market_total_output_crawler import (
    NonMarketTotalOutputCrawler,
    crawl_non_market_total_output_for_date,
...
# 设置日志记录器
logger = setup_logger('test_non_market_total_output')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'non_market_total_output']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = NonMarketTotalOutputCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算非市场机组实时总出力字段的最小值和最大值
            if 'non_market_total_output' in df.columns:
                logger.info(f"non_market_total_output 范围: {df['non_market_total_output'].min()} 至 {df['non_market_total_output'].max()}")
