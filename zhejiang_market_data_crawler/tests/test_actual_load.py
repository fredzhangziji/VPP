...
from crawlers.actual_load_crawler import (
    ActualLoadCrawler, 
    crawl_actual_load_for_date,
...
# 设置日志记录器
logger = setup_logger('test_actual_load')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'actual_load']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = ActualLoadCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            if 'actual_load' in df.columns:
                logger.info(f"actual_load 范围: {df['actual_load'].min()} 至 {df['actual_load'].max()}")
