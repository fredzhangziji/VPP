...
from crawlers.actual_solar_output_crawler import (
    ActualSolarOutputCrawler, 
    crawl_actual_solar_output_for_date,
...
# 设置日志记录器
logger = setup_logger('test_actual_solar_output')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'actual_solar_output']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = ActualSolarOutputCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算光伏实时总出力字段的最小值和最大值
            if 'actual_solar_output' in df.columns:
                logger.info(f"actual_solar_output 范围: {df['actual_solar_output'].min()} 至 {df['actual_solar_output'].max()}")
