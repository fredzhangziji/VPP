...
from crawlers.non_market_solar_output_crawler import (
    NonMarketSolarOutputCrawler,
)

# 目标表名和字段名
TABLE_NAME = 'power_market_data'
FIELD_NAME = 'non_market_solar_output'

# 设置日志记录器
logger = setup_logger('test_non_market_solar_output')
...
async def test_single_date():
    ...
    try:
        crawler = NonMarketSolarOutputCrawler(target_table=TABLE_NAME)
...
async def test_data_persistence():
    ...
    try:
        ...
        if FIELD_NAME in df.columns and FIELD_NAME in db_df.columns:
            ...
            if value_matches:
                logger.info("所有非市场光伏实时总出力值匹配")
            else:
                logger.warning("非市场光伏实时总出力值不完全匹配")
