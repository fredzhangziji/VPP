...
from crawlers.system_backup_crawler import (
    SystemBackupCrawler, 
    crawl_system_backup_for_date,
...
# 设置日志记录器
logger = setup_logger('test_system_backup')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'system_reserve']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = SystemBackupCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算系统备用字段的最小值和最大值
            if 'system_reserve' in df.columns:
                logger.info(f"system_reserve 范围: {df['system_reserve'].min()} 至 {df['system_reserve'].max()}")
