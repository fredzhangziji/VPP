...
from crawlers.actual_total_generation_crawler import (
    ActualTotalGenerationCrawler,
)

# 目标表名和字段名
TABLE_NAME = 'power_market_data'
FIELD_NAME = 'actual_total_generation'

# 设置日志记录器
logger = setup_logger('test_actual_total_generation')
...
async def main():
    try:
        logger.info("===== 测试发电实时总出力爬虫 =====")
        # 检查数据库结构
        assert check_db_config(), "数据库结构检查未通过"
        # 测试单日数据
        test_date = '2025-06-02'
        crawler = ActualTotalGenerationCrawler(target_table=TABLE_NAME)
