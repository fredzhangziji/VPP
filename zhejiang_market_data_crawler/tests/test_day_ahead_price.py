...
from crawlers.day_ahead_price_crawler import (
    DayAheadPriceCrawler, 
    crawl_day_ahead_price_for_date,
...
# 设置日志记录器
logger = setup_logger('test_day_ahead_price')
...
def check_db_config():
    ...
    try:
        ...
        if TABLE_NAME in tables:
            ...
            # 检查必要的列是否存在
            required_columns = ['date_time', 'day_ahead_price']
...
async def test_single_date():
    ...
    try:
        # 创建爬虫实例
        crawler = DayAheadPriceCrawler(target_table=TABLE_NAME)
        ...
        if not df.empty:
            ...
            # 计算日前市场出清负荷侧电价字段的最小值和最大值
            if 'day_ahead_price' in df.columns:
                logger.info(f"day_ahead_price 范围: {df['day_ahead_price'].min()} 至 {df['day_ahead_price'].max()}")
...
async def test_date_range():
    ...
    # 注意：每天数据的时间范围是从当天00:30到第二天00:00，因此2025-06-03的数据会包含2025-06-04 00:00
